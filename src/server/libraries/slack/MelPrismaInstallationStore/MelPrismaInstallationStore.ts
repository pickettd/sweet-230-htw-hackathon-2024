...
import PrismaInstallationStoreArgs from './MelPrismaInstallationStoreArgs'
import {
  DeleteInstallationCallbackArgs,
  FetchInstallationCallbackArgs,
  StoreInstallationCallbackArgs,
} from './MelPrismaInstallationStoreCallbackArgs'

export default class MelPrismaInstallationStore implements InstallationStore {
  ...
  private onFetchInstallation: (
    args: FetchInstallationCallbackArgs,
  ) => Promise<void>

  private onStoreInstallation: (
    args: StoreInstallationCallbackArgs,
  ) => Promise<void>
  ...
  public constructor(options: PrismaInstallationStoreArgs) {
    ...
    this.logger =
      options.logger !== undefined ? options.logger : new ConsoleLogger()
    this.historicalDataEnabled =
      options.historicalDataEnabled !== undefined
        ? options.historicalDataEnabled
        : true
    this.onFetchInstallation =
      options.onFetchInstallation !== undefined
        ? options.onFetchInstallation
        : async _ => {}
    this.onStoreInstallation =
      options.onStoreInstallation !== undefined
        ? options.onStoreInstallation
        : async _ => {}
    this.onDeleteInstallation =
      options.onDeleteInstallation !== undefined
        ? options.onDeleteInstallation
        : async _ => {}

    this.logger.debug(
      `PrismaInstallationStore has been initialized (clientId: ${this.clientId})`,
    )
  }
      ...
      const installation: any = {
        ...
        user: {
          ...
          expiresAt: row.userTokenExpiresAt
            ? Math.floor(row.userTokenExpiresAt.getTime() / 1000)
            : undefined,
        ...
        bot:
          row.botId && row.botUserId && row.botToken
            ? {
                ...
                expiresAt: row.botTokenExpiresAt
                  ? Math.floor(row.botTokenExpiresAt.getTime() / 1000)
                  : undefined,
        ...
        incomingWebhook: row.incomingWebhookUrl
          ? {
              ...
              configurationUrl:
                row.incomingWebhookConfigurationUrl || undefined,
            }
          : undefined,
